...
**CRÍTICOS**
- **Precio de toldos con motor/cobertor (C1+C2):** al distribuidor se le cobraba de más. (a) `autoAddChaponItem` (cobertor) y `autoAddMotorToldoItem` (motor) ahora aplican `descDistribuidorPct` (claves 'toldos' y 'motores'), igual que el item principal del toldo — antes se agregaban a precio de lista sin descuento. (b) El motor de toldo usaba `getCoefCuotas()` (~1,35) por un comentario equivocado que lo confundía con los motores de RIEL (`aplicarSoloCoef`); `motores` es `aplicarFV:true`, así que ahora usa `getFV()`, igual que el alta manual y la vista previa (L5925-5926).
- **XSS en planillas de fábrica y presupuesto (C3+C4):** campos de texto libre del distribuidor (cliente, nombre de tela manual, color, referencia) se imprimían sin escapar → un texto malicioso podía ejecutar código en la sesión de fábrica/admin (cruce distribuidor→admin). Se envolvieron con `esc()`: en `openPlanillaConfeccion` (cliente, nombreTela, telaLabel); en `openPlanilla` filas Roller HOJA 1/2 (it.color, it.ref) + accesorios (colorStr, refStr) + refLabel de rieles/confección; y en el presupuesto/`openDoc` (it.color de item libre, it.ref de renglones Roller y pedido). Verificado con barrido: no quedan renders crudos de it.color/it.ref/refLabel/nombreTela/cliente (los `telaLabel` restantes son etiquetas fijas de producto).
- **Doble conversión de cotización (C5):** el botón 📦 seguía visible tras convertir → se podía crear un 2º pedido con el mismo número y duplicar la deuda. Ahora el botón se oculta si la cotización ya está 'Aprobada' y `convertToOrder` aborta si la cotización ya fue convertida (estado Aprobada o ya existe un pedido con ese `cotizacionOrigen`).
- **Numeración de pedido vinculado (C6):** "Nuevo Pedido" + 🔗 Vincular tomaba un número secuencial nuevo en vez de derivarlo de la cotización (ej. COT-0657 → PED-0646). Ahora, en la rama draft de `confirmarPedido`, si hay cotización origen usa `PED-<base>` igual que la conversión normal, y aborta si esa cotización ya tiene un pedido.
- **Sync offline pisaba el estado del servidor (C7):** al volver la conexión, la copia offline re-subía el pedido completo (incluido `estado`), revirtiendo lo que fábrica/admin hubiera avanzado (ej. "En Fabricación" → "Pendiente"). `saveDocToSupabase(doc, {preserveServerEstado:true})` (usado por `syncPendingDocs`) ahora consulta el estado remoto y lo respeta; el contenido editado sí sube.

**IMPORTANTES**
- **Descuento adicional (I8):** `editDoc` no restauraba `_descuentoActivo`/campos desde `doc.descuento` (se perdía al guardar) y `resetForm` no lo reseteaba (se "filtraba" al siguiente presupuesto). Ahora `editDoc` lo restaura (modo %/$ , monto/pct, concepto) y `resetForm` lo desactiva y limpia. (No aplica a distribuidores — V418.)
- **Corporativo (I9):** `editDoc` no restauraba `_corpActivo` desde `doc.corporativo` (podía perder/filtrar el 25% OFF). Ahora sí.
- **Vínculo motor↔cortina y motorizado (I10):** `addRow` no guardaba `motor`, `cortina_asignada` ni `motorizado` en el JSON del item, así que se perdían al re-editar/re-guardar (se persistían en DB pero no en el form). Ahora se incluyen.
- **Planilla tras recargar (I11):** la planilla leía `it.confData.*` (que no existe tras recargar) para el tag ⚡ Motorizado y la combinación de tramos del riel Celtic. Ahora usa fallback al campo plano (`it.motorizado`, `it.celticTramos`, `it.motorLabel`), que sí persiste.
- **Descuento de instalación (I12):** la clave 'instalacion' del panel de descuentos por distribuidor no se aplicaba nunca. Nuevo `descInstalacionPct()` (lee `ME.descuentos['instalacion']`) aplicado en `getInstalTotal` → el control ahora funciona. (Decisión del dueño; la instalación viene apagada por defecto para distribuidores, riesgo mínimo.)
- **Necesita Revisión / cuenta corriente (I13):** tras inspeccionar el trigger en Supabase se **revirtió** este cambio. `esModifControlada` NO incluye 'Necesita Revisión' a propósito: es un estado AMBIGUO (se le puede poner a un pedido que nunca se aprobó), y el trigger `cc_ajustar_modif_pedido` también lo excluye — justamente para no imputarle deuda a un pedido sin aprobar. Front y trigger quedan alineados. El caso raro "aprobado → revisión → cambia total → re-aprueba" queda para ajuste manual desde la CC del admin (agregar el estado al trigger sería incorrecto: cobraría pedidos sin aprobar).
- **Borrado de movimiento de CC (I14):** `eliminarMovCC` ahora impide borrar a mano el débito de un pedido ACTIVO (descuadraría la cuenta); permite borrar huérfanos o de pedidos cancelados. Para revertir la deuda de un pedido vivo hay que cancelarlo/eliminarlo (ajusta la CC solo).

**Cuenta corriente (Supabase) — VERIFICADO OK (2026-07-01):** se inspeccionaron los dos triggers de `documentos`. `cc_imputar_pedido` solo imputa al PASAR a 'Aprobado' (`NEW.estado='Aprobado' and NEW.estado is distinct from OLD.estado`) con guarda `not exists (... cc_movimientos where pedido_id=NEW.id and tipo='debito')` → **idempotente, no hay doble cobro** al re-aprobar; y `delete` del débito al pasar a 'Cancelado' → **reversa**. `cc_ajustar_modif_pedido` postea la diferencia (débito/crédito) cuando cambia el `total` de un pedido en estado post-aprobación (`Aprobado`/`En Fabricación`/`Parcialmente terminado`), excluye pedidos sin aprobar y tiene `exception when others then return NEW` (nunca bloquea el guardado). **Sin cambios necesarios en la base.**

## V430 — 2026-06-29 — Cuenta corriente: el admin puede eliminar cualquier movimiento (con aviso)

A pedido del dueño (para limpiar errores, ej. débitos de pedidos de prueba). Antes el botón ✕ del ledger (`renderCCLedger`) solo aparecía para créditos y débitos manuales (`tipo==='credito' || (tipo==='debito' && !pedido_id)`); ahora aparece para TODOS los movimientos. Como el ledger es admin-only (requiere `_ccSelDist`, gestión de distribuidores del admin), el borrado queda restringido a admin. `eliminarMovCC(id)` ahora, si el movimiento es el débito automático de un pedido (`tipo==='debito' && pedido_id`), muestra un confirm reforzado: recomienda CANCELAR/eliminar el pedido (la CC se ajusta sola por trigger) y advierte que el borrado manual puede descuadrar; para el resto, confirm normal. Verificado en navegador: el débito de pedido ahora tiene ✕ y dispara el aviso fuerte; los créditos, confirm normal. node --check OK, sin errores de consola. NOTA: borrar a mano un débito de pedido NO reabre el pedido ni reimputará solo; lo correcto sigue siendo cancelar/eliminar el pedido.
...
## V416 — 2026-06-29 — Fix: la vista previa de motores mostraba $1 (Somfy) aunque se agregaba con el precio real

Gastón notó que al **seleccionar** un motor Somfy en el cotizador la vista previa decía **$1**, pero al **Agregar** se cargaba con el precio real. Causa: dos fuentes de precio distintas. La vista previa (`qaCalcPrecio`, rama `tipo:'unitario'`, ~línea 5844) leía solo el mapa interno `TELAS.motores.precios[modelo]`, donde los Somfy tubulares y controles están en **$1** (placeholders V380: `Somfy 6/28 RF/WD`, `Somfy 10/28 RF`, `Control 1/5 Somfy`). En cambio el Agregar (~6598) usa `getPrecioMotor()`, que primero busca en la **Lista de Precios** (`getInsumos()` por nombre + cat/subcat 'Motores') y solo cae al mapa si no lo encuentra. Fix: la vista previa de motores (`tela==='motores'||'motor_somfy_wt'||'motor_somfy_mv35'`) ahora usa `getPrecioMotor(modeloSel)` como fuente primaria, igual que el Agregar → preview = lo que se agrega. node --check OK. NOTA: los 5 Somfy siguen en $1 a nivel default (INSUMOS_DEFAULT + TELAS.precios); el precio real vive en la Lista de Precios de cada uno. Pendiente del dueño: pasar los precios reales de esos 5 para fijarlos como default (hoy es una tarea pendiente de "poner precios reales a los motores/controles Somfy $1").
...
## V410 — 2026-06-28 — Asistente IA (Claude Sonnet) — base

Se agregó un asistente con IA. Arquitectura segura: una función serverless en Netlify (`netlify/functions/ai.mjs`) hace de intermediario — guarda la API key de Anthropic SOLO como variable de entorno (`ANTHROPIC_API_KEY`, nunca en el código público), valida que quien llama sea un usuario logueado (token JWT de Supabase, vía `/auth/v1/user`) antes de gastar saldo, y proxea a Claude Sonnet (`claude-sonnet-4-6`). En el frontend, un botón flotante "✨ Asistente" (visible solo dentro de la app logueada) abre un chat que arma el contexto (catálogo de productos + presupuestos visibles según el rol, con precios reales) y conversa. Capacidades v1: resumir presupuestos, redactar el mensaje al cliente (WhatsApp/mail), responder preguntas. La IA NO inventa precios (usa solo los datos provistos). La "versión económica que crea un presupuesto nuevo con precios exactos" queda para fase 2 (por ahora la IA explica el cambio de telas USA→EURO / Sunscreen→Mesh).

`netlify.toml` agregado solo para indicar la carpeta de funciones (no define build/publish, así el sitio estático sigue igual). El chat es un bloque `<script>` independiente al final de index.html (no toca nada existente). Pendiente del lado del dueño para activarlo: crear cuenta Anthropic + API key + cargar crédito + setear `ANTHROPIC_API_KEY` en Netlify; hasta entonces el asistente responde "todavía no está configurado" (HTTP 503). node --check OK (función + scripts); UI verificada en navegador (botón aparece solo logueado, modal abre, contexto se arma).
...
Implementado como key nuevo `blackout_euro` (100% aditivo: NO se tocó el key `blackout` existente, su cálculo, sus ~28 referencias, ni las cotizaciones ya guardadas, que siguen con tela:`blackout`). Touchpoints: TELAS (label USA + entry EURO), insumo `tela_blackout_euro` ($5.700), getTelaCostoId (mapeo crítico — sin él el EURO cobraría la tela del USA), _descKeyForItem (descuento compartido), TIPOS_ROLLER, telasConDescuento, tiposConEje (planilla: rollo + eje + zócalo + mecanismo + cadena + soportes), TELA_LABELS (`Roller Blackout`, sin sufijo), BRAND_NAMES (`EURO 520`), edición de color en línea, badge `EURO 520` en la planilla (openPlanilla), y rótulos internos (dropdown de motores, histograma de stats, badge CÁLCULO en Lista de Precios). El stock del EURO es propio (otra tela): si no hay partida cargada, aparece como "faltante" sin romper nada (no descuenta de la del USA). Las tablas PRICE_TABLES/lookupPrice no se tocaron (código muerto, 0 llamadas; el precio roller real sale de calcCostoRoller). Verificado end-to-end en el navegador: el desplegable muestra ambos ("USA" y "EURO" juntos en Cortinas); EURO $181.355 < USA $226.636 (2000×1500); presupuesto del cliente con nombre "Roller Blackout" + gris "EURO 520" sin filtrar sufijos; planilla con insumos idénticos al USA + badge "EURO 520" solo en el EURO; descuento de distribuidor resuelve a la categoría 'blackout'. node --check OK.
...
Tras V400 (símil madera), se barrieron las 157 combinaciones producto×subtipo como distribuidor con un descuento DISTINTO por categoría, comparando precio mostrado vs agregado. Resultado: el bug de símil madera NO está replicado en el resto (cortinas, sunscreen, doble, duo, confección, cambio de tela, bandas, paneles, venecianas, rieles, motores, telas, ejes/tubos/ángulos/cenefas/barrales) — todos coinciden. Se detectó UN caso más del mismo tipo: los Accesorios de Toldos (chapón cubre toldos) mostraban precio SIN descuento (tela accesorios_toldos no mapeaba a categoría) pero al agregar la tela pasa a cobertor_toldo → categoría toldos (con descuento). Fix: `_descKeyForItem` ahora mapea accesorios_toldos → toldos, igual que el chapón al agregar. Así mostrar y agregar coinciden, y los accesorios de toldos (chapón, calotas, zócalo, tapas) toman consistentemente el descuento de toldos del distribuidor (el chapón YA lo cobraba; ahora también se muestra bien y se extiende al resto de accesorios de toldos). Verificado: 0 descalces de descuento; solo queda 1 diferencia de $1 por redondeo en cambio de tela (despreciable).

## V400 — 2026-06-25 — Fix descuento distribuidor: Veneciana Símil Madera (mostrar ≠ agregar)

En cuentas distribuidor, al cotizar una Veneciana "Simil Madera 50mm" el precio mostrado difería del precio al agregar (el agregado salía más caro). Causa: la clave interna del subtipo es `ven_sim50` (tiene "sim", no "simil"). `_descKeyForItem` decidía la categoría de descuento buscando la palabra "simil": al MOSTRAR usaba la clave `ven_sim50` → no encontraba "simil" → caía en el descuento de ALUMINIO (`ven_alum`); al AGREGAR usaba la etiqueta "Simil Madera" → sí encontraba "simil" → usaba símil madera (`ven_simil`). Con % distintos por categoría, mostrar y agregar daban precios distintos. Fix: `_descKeyForItem` ahora reconoce también las CLAVES de subtipo (`ven_sim`/`ven_alum`/`ven_mad`), no solo las etiquetas. Así mostrar y agregar usan la MISMA categoría (símil madera, la correcta). Verificado: con alum 30% / símil 20%, antes display=30% y add=20% (descalce); ahora ambos 20% y el precio coincide. Aluminio y Madera ya funcionaban (sus claves contienen alum/mad). El precio que se AGREGABA siempre fue el correcto; lo que estaba mal era el número mostrado (aplicaba el descuento de aluminio de más).
...
## V396 — 2026-06-20 — Export PDF en TELÉFONO: generar el PDF como imagen A4 (html2canvas + jsPDF), sin depender del motor de impresión de iOS

Tras 5 iteraciones (V390-V395) el iPhone seguía exportando mal (cortado / achicado / "copia la pantalla"): el **motor de impresión de WebKit en iPhone no respeta el layout A4** que se le da, mientras que en **tablet (iPad) y compu sí** funciona el método de impresión. Cambio de enfoque definitivo, **solo para teléfonos**: en vez de imprimir, se **genera el PDF como imagen** — el documento se renderiza en un iframe de ancho A4 (794px), **html2canvas** lo captura exactamente como en la compu y **jsPDF** lo escribe en hojas A4. El resultado es **idéntico en cualquier dispositivo** porque no pasa por el motor de iOS. Detección de teléfono (NO tablet): `iPhone/iPod`, `Android + Mobile`, o pantalla ≤500px; iPad/tablets y desktop quedan **intactos** en su método de impresión actual. Las librerías (`html2canvas` 1.4.1 + `jsPDF` 2.5.1) se alojan en el propio sitio (`/vendor`, ~560KB) y se cargan perezosamente solo al exportar desde el teléfono; si no cargan (sin internet) cae al método de impresión anterior. Las tarjetas de financiación se aplanan a degradado lineal (las `radial-gradient` se rasterizaban como círculos). Verificado end-to-end EN LOCAL (esto sí se puede, a diferencia del motor de iOS): simulando un teléfono de 390-480px y usando el CSS REAL de la app, el PDF sale A4 perfecto — logo con su fuente, tabla de 8 columnas completa sin corte, TOTAL visible, tarjetas con fondo liso sin círculos, 1 página. La rama mobile de impresión (V395, iframe 715px) queda como fallback para tablet y para teléfono sin internet.

## V395 — 2026-06-20 — Export PDF iPhone: arreglar el corte a la derecha (ancho A4 real) + círculos negros de las cards

El iframe de V394 ya forzaba un viewport ancho y nombre correcto, pero el ancho era **900px** y el iPhone **no escala** el contenido a la hoja: usa el viewport como ancho de layout y lo que sobra del A4 **lo corta** (por eso la tabla/Total salían cortados a la derecha). Fix: el iframe ahora mide **715px** = el área imprimible de una A4 (190mm ≈ 718px @96dpi, con un mini margen de seguridad), así el documento se arma exactamente al ancho de la hoja — ni cortado ni achicado al teléfono. Verificado en preview a 390px: doc y tabla (8 col) = 715px, entran sin desbordar. Segundo problema: las **tarjetas de financiación** (cuotas / contado) usan fondos con `radial-gradient` en capas con `transparent`, que el motor de impresión de iOS **rasteriza como círculos negros** tapando los números. Fix: se les puso clase (`rm-fincard-cuotas` / `rm-fincard-contado`) y en el CSS del PDF se sobreescribe el fondo a un **degradado lineal plano** (`linear-gradient`), que imprime limpio. Verificado: en el iframe las cards quedan sin `radial-gradient` (solo el lineal). Escritorio sin cambios (las cards en pantalla siguen con su degradado original). Falta confirmar en iPhone real.

## V394 — 2026-06-20 — Export PDF iPhone: forzar ancho A4 con un iframe de viewport propio (900px)

El PDF del iPhone seguía saliendo "copiando el tamaño de la pantalla del teléfono" (≈390px) aunque las reglas mobile ya estaban limitadas a `screen` (V393). Causa de fondo: al imprimir, **el navegador del celular usa el ancho de la pantalla del teléfono como viewport de layout** — un `#rm-print-root` a 190mm igual desbordaba/se achicaba porque el documento se sigue armando a 390px. La única forma real de forzar A4 desde un celular es darle al documento **su propio viewport ancho**: ahora `printDoc` (rama mobile) renderiza la cotización dentro de un **iframe de 900px de ancho fijo** (fuera de pantalla, `left:-10000px`) con `<meta viewport content="width=900">` + `html,body{width:900px}` y todos los estilos de la app; dentro de ese iframe las reglas de celular **no aplican** (900 > 600) y el doc se arma exactamente como en la compu (A4). La impresión se dispara **desde adentro del propio iframe** (su `window.print()` en el `onload`), no desde la página padre — así el PDF sale del iframe (ancho A4 + nombre correcto `COT-XXX Cliente Roller Motion`), no de la pantalla del teléfono. Esto corrige las dos fallas de V390 (iframe de 1px + print disparado desde el padre → imprimía la página). Escritorio sin cambios. Sintaxis verificada; falta confirmar el render en un iPhone real (no se puede simular el motor de impresión de iOS localmente).
...
## V390 — 2026-06-20 — Fix: exportar/imprimir cotización en PDF desde el iPhone salía en blanco

En el celular (iPhone, Safari/Chrome iOS) exportar una cotización a PDF daba **páginas en blanco** (confirmado: 5 hojas, 6KB, sin contenido). Causa: la rama mobile de `printDoc` imprimía la ventana actual con un truco de `@media print` (`body *{visibility:hidden}` + `#doc-content{visibility:visible;position:absolute}`), y ese patrón **no renderiza en iOS/WebKit** (el contenido queda invisible aunque ocupe el espacio → hojas en blanco). Fix: en mobile ahora se arma el documento limpio (mismo HTML+estilos que usa la rama de escritorio) dentro de un **iframe oculto** (flujo normal, sin `visibility`/`position:absolute`) y se imprime desde ahí con `iframe.contentWindow.print()` — método confiable en iPhone/iPad, sin depender de popups. La rama de escritorio queda intacta. Verificado el mecanismo en preview: el iframe recibe el contenido + estilos y renderiza con altura real (no 0); falta confirmar el render en un iPhone real.
